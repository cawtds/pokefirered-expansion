...
const u32 gFieldEffectObjectPic_ORASDowsingRed[] = INCBIN_U32("graphics/field_effects/pics/oras_dowsing_red.4bpp");
const u32 gFieldEffectObjectPic_ORASDowsingGreen[] = INCBIN_U32("graphics/field_effects/pics/oras_dowsing_green.4bpp");
const u16 gFieldEffectPal_ORASDowsing[] = INCBIN_U16("graphics/field_effects/palettes/oras_dowsing.gbapal");

static const struct SpriteFrameImage sPicTable_ORASDowsingRed[] = {
    overworld_ascending_frames(gFieldEffectObjectPic_ORASDowsingRed, 2, 4),
};

static const struct SpriteFrameImage sPicTable_ORASDowsingGreen[] = {
    overworld_ascending_frames(gFieldEffectObjectPic_ORASDowsingGreen, 2, 4),
};
...
const struct SpriteTemplate gFieldEffectObjectTemplate_ORASDowsingRed = {
    .tileTag = TAG_NONE,
    .paletteTag = FLDEFF_PAL_TAG_ORAS_DOWSE,
    .oam = &gObjectEventOam_ORASDowse,
    .anims = sAnimTable_ORASDowsing,
    .images = sPicTable_ORASDowsingRed,
    .callback = UpdateORASDowsingFieldEffect,
};

const struct SpriteTemplate gFieldEffectObjectTemplate_ORASDowsingGreen = {
    .tileTag = TAG_NONE,
    .paletteTag = FLDEFF_PAL_TAG_ORAS_DOWSE,
    .oam = &gObjectEventOam_ORASDowse,
    .anims = sAnimTable_ORASDowsing,
    .images = sPicTable_ORASDowsingGreen,
    .callback = UpdateORASDowsingFieldEffect,
};
...
u32 FldEff_ORASDowsing(void)
{
    ...
    if (gPlayerAvatar.gender == MALE)
        spriteId = CreateSpriteAtEnd(&gFieldEffectObjectTemplate_ORASDowsingRed, fPlayerX, fPlayerY, 1);
    else
        spriteId = CreateSpriteAtEnd(&gFieldEffectObjectTemplate_ORASDowsingGreen, fPlayerX, fPlayerY, 1);
...
void UpdateDowseState(struct Sprite *sprite)
{
    ...
    if (HiddenItemIsWithinRangeOfPlayer(gMapHeader.events, TASK_NONE) == TRUE)
    {
        s8 distX = sprite->tItemDistanceX;
        s8 distY = sprite->tItemDistanceY;
        enum Direction directionToItem = CARDINAL_DIRECTION_COUNT;
        enum Direction playerDirToItem = GetPlayerDirectionTowardsHiddenItem(distX, distY);
        if (playerDirToItem != DIR_NONE)
            directionToItem = sClockwiseDirections[GetPlayerDirectionTowardsHiddenItem(distX, distY) - 1];
